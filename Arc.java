	...
	public ArrayList<Arc> generateArcs() {
		...
		for(Wall wallCheck: Wall.walls) {
			...
			if(rebound != null && isBehindRebound(wallPoints, wallOrient)) {
				// wall is behind rebound and should be skipped
				continue;
			}
			if(min.angle == max.angle) {
				// wall is in line with pos and should be skipped
				continue;
			}
		...
		for(int i = 0; i < angs.size(); i++) {
			...
			/*
			if(next != null && check.angle == next.angle) {
				//System.out.println("HERE!!!!");
				if(next.angle == check.angle) {
					p.pushStyle();
						p.stroke(255, 0, 0);
						//p.line(pos.x, pos.y, pos.x + Main.time * (float) Math.cos(check.angle), pos.y + Main.time * (float) Math.sin(check.angle));
					p.popStyle();
				}
				
				// check if next angle is equal to checks
				AngVect closest = null;
				float dist = -1;
				int count;
				for(count = 0; i + count < angs.size() && angs.get(i + count).angle == check.angle; count++) {
					// go through all AngVects in angs with same angle
					AngVect newClosest = angs.get(i + count);
					if(newClosest.isMin) {
						// must be min to consider
						float newDist = pos.distanceTo(newClosest.pos);
						if(newDist < dist || dist == -1) {
							closest = newClosest;
							dist = newDist;
						}
					} else {
						// max so wall ended, remove from started list
						started.remove(newClosest.link);
					}
				}
				...
				if(closest != null) {
					// found best candidate, similar to interupt
					if(!current.wall.intersects(pos, closest.pos)) {
						// make sure closest pos is behing current wall
						current = closest;
					}
				} else {
					// all walls with same angle were max, current becomes next AngVect after these
					if(i + count < angs.size()) {
						current = angs.get(i + count);
					}
				}
				
				// increment i to skip over all AngVects with this angle
				i += count - 1;
				// END OF repeated angle case and this iteration of the loop
			} else */if(check == current.link) {
				...
				if(best != null) {
					...
					subArcs.add(new Arc(lastAngle, next.angle, pos));
					
					current = next;
					lastAngle = next.angle;
